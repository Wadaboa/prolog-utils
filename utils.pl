...
not_member(V, X) :- not(member(V, X)).

% Membership by selection
member_by_if(X, [Y|RS]) :-
    (
	X = Y -> true;
	member_by_if(X, RS) -> true;
	false
    ).

% Number of occurences of an integer in a list
occurrences([], _, O) :- O is 0.
occurrences([I|T], I, O) :-
	occurrences(T, I, P),
	O is P + 1,
	!.
occurrences([_|T], I, O) :- occurrences(T, I, O).

% Length of a list
list_length([], L) :- L is 0.
list_length([_|T], L) :-
	list_length(T, M),
	L is M + 1.
...
append([], Y, Y).
append([XH|XT], Y, [XH|Z]) :- append(XT, Y, Z).
...
% List euclidean norm
squared_euclidean_norm([], N) :- N is 0.
...
split_two([], [], []).
split_two([A], [A], []).
split_two([A, B|T], [A|P1], [B|P2]) :- split_two(T, P1, P2).
...
% Merge two sorted sublists into a one sorted list
merge_two([], L, L).
merge_two(L, [], L).
...
% Set type
set([], []).
set([H|T], [H|S]) :- not_member(H, T), !, set(T, S).
set([_|T], S) :- set(T, S).
...
% Enqueue operation
enqueue(queue(Q1), E, queue(Q2)) :- not(queue(E)), append(Q1, [E], Q2), !.

% Dequeue operation
dequeue(queue(Q1), queue(Q2)) :- remove_prefix(Q1, 1, Q2), !.
